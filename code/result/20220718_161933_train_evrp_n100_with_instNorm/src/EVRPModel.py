...
class EVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        node_distance = reset_state.node_distance
        # shape: (batch, problem)
        node_xy_distance = torch.cat((node_xy, node_distance[:, :, None]), dim=2)
        # shape: (batch, problem, 3)

        self.encoded_nodes = self.encoder(depot_xy, node_xy_distance)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, state.battery, ninf_mask=state.ninf_mask)
...
class EVRP_Encoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_distance):
        # depot_xy.shape: (batch, 1, 2)
        # node_xy_demand.shape: (batch, problem, 3)

        embedded_depot = self.embedding_depot(depot_xy)
        # shape: (batch, 1, embedding)
        embedded_node = self.embedding_node(node_xy_distance)
